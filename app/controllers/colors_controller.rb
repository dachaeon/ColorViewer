class ColorsController < UIViewController
  def viewDidLoad
    ...
    ["red", "green", "blue", "purple"].each_with_index do |color_text, index|
      ...
      button.sizeToFit
      button.frame = [
                      [20+index*(button_width + 10), @label.frame.origin.y + button.frame.size.height + 30],
                      [70, button.frame.size.height]
                     ]
  ...
  def tap_purple
    controller = ColorDetailController.alloc.initWithColor(UIColor.purpleColor)
    self.navigationController.pushViewController(controller, animated: true)
  end
